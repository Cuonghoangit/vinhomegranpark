...
public class AlgoFHM {
	...
	* the idea is to always reuse the same buffer to reduce memory usage. */
	 ...
	 */
	public void runAlgorithm(String input,String threshHoldPath, String output) throws IOException {
		// reset maximum
		...
		List<Integer> AllItemList = new ArrayList<Integer>();
	   	...
	   	insertionSort(AllItemList, mapItemToTWU);
		...
		for(Integer item: mapItemToTWU.keySet()){
			...
			if(mapItemToTWU.get(item) >= mapItemWithSMUofItem.get(item)){
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				for(int i=0; i <items.length; i++){
					...
					if(mapItemToTWU.get(pair.item) >= mapItemWithSMUofItem.get(pair.item)){
						// add it
...
//					int remain = remainingUtility; // FOR OPTIMIZATION
		...
		fhm(itemsetBuffer, 0, null, listOfUtilityLists, 0);
	...
	private void fhm(int [] prefix,
			int prefixLength, UtilityList pUL, List<UtilityList> ULs, long prefixMMU)
			throws IOException {
		
		// For each extension X of prefix P
		for(int i=0; i< ULs.size(); i++){
			UtilityList X = ULs.get(i);
			long valueOfMMUPrefix=prefixMMU;
			if (valueOfMMUPrefix==0) {
				valueOfMMUPrefix = mapItemToThreshhold.get(X.item);
			}else {
				valueOfMMUPrefix = prefixMMU;
				long valueMMUofX = mapItemToThreshhold.get(X.item);
				if (valueOfMMUPrefix>valueMMUofX) {
					valueOfMMUPrefix = valueMMUofX;
				}
			}
			long valueOfSMU = valueOfMMUPrefix;
			for (int j=i+1; j < ULs.size(); j++) {
				int Y = ULs.get(j).item;
				long valueMMUofY = mapItemToThreshhold.get(Y);
				if(valueMMUofY<valueOfSMU) {
					valueOfSMU = valueMMUofY;
				}
			}
			
			// If pX is a high utility itemset.
			// we save the itemset:  pX 
			if(X.sumIutils >= valueOfMMUPrefix){
				// save to file
			...
			if(X.sumIutils + X.sumRutils >= valueOfSMU){
				...
				for(int j=i+1; j < ULs.size(); j++){
					...
					UtilityList temp = construct(pUL, X, Y, valueOfSMU);
					if(temp != null) {
						exULs.add(temp);
					}
				}
				...
				fhm(itemsetBuffer, prefixLength+1, X, exULs, valueOfSMU); 
			}
		}
		MemoryLogger.getInstance().checkMemory();
	}
	 ...
	 */
	private UtilityList construct(UtilityList P, UtilityList px, UtilityList py, long valueOfSMU) {
    ...
    public static void insertionSort(List<Integer> items, Map<Integer,Long> MapItemToTWU){
		...
		for(int j=1; j< items.size(); j++){
			...
			long comparison = MapItemToTWU.get(itemI) - MapItemToTWU.get(itemJ);
			...
			while(comparison > 0){
				...
				itemI = items.get(i);
				comparison = MapItemToTWU.get(itemI) - MapItemToTWU.get(itemJ);
				// if the twu is equal, we use the lexicographical order to decide whether i is greater
				// than j or not.
				if(comparison == 0){
					comparison = itemI - itemJ;
